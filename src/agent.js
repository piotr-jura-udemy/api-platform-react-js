...
export const requests = {
  ...
  upload: (url, file, progressCallback, secured = true) => {
    const uploaded = superagent.post(`${API_ROOT}${url}`).attach('file', file)
      .use(tokenPlugin(secured))
    uploaded.on('progress', e => progressCallback(e.percent));

    return uploaded.then(responseBody);
  },
